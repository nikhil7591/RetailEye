...
import { LayoutDashboard, History, FileText, Bell, Settings, Eye, ScanLine, X } from "lucide-react";
...
export function Sidebar({ isOpen, onClose }) {
  ...
  const navItems = [
    ...
    { name: "Analysis", href: "/dashboard/analysis", icon: ScanLine },
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex h-screen w-[260px] shrink-0 flex-col border-r border-[#E2E8F0] bg-[#FFFFFF] transition-transform duration-300 ease-in-out lg:static lg:translate-x-0",
      ...
      >
        <div className="flex h-[72px] items-center justify-between px-6">
          ...
          <button
            onClick={onClose}
            className="lg:hidden p-1.5 rounded-lg text-[#64748B] hover:text-[#0F172A] hover:bg-[#F1F5F9] transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-4 mb-2 flex flex-col gap-3">
